...
def vnos():
    ...
    odgovor = input('Koliko je ' + str(x) + '*' + str(y) +':')
    cas = time.perf_counter() - zacetek
    if int(odgovor) == (x*y):
        print('Čestitam! Odgovor je pravilen. Za odgovor si potreboval', cas, 's.')
...
b = int(input('Drugo število: '))
print('Računam ...')
cas = random.uniform(1, 5)
time.sleep(cas)
print('Uf, je bilo težko. V', cas,'sek so se moji čipi pošteno ogreli. Rezultat je:', a*b)
# =====================================================================@018393=
# 3. podnaloga
# **Misleči stroj II**
# 
...
def stroj(): 
    ...
    neg = (izracun/10 + 2)
    poz = (izracun/10)
    if a < 0 or b < 0 and (neg <= 10 or poz <= 10):
        time.sleep(neg)
        print('Uf, je bilo težko. V', neg,'sek so se moji čipi pošteno ogreli. Rezultat je:', izracun)
    elif neg <= 10 and poz <= 10:
        time.sleep(poz)
        print('Uf, je bilo težko. V', poz,'sek so se moji čipi pošteno ogreli. Rezultat je:', izracun)
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjozMzA3LCJwYXJ0IjoxODM5MX0:1gGOCW:F41U9DZTalHcRo-SZETKnxxEDyw'
        try:
            pass
        except:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjozMzA3LCJwYXJ0IjoxODM5Mn0:1gGOCW:78rF-8Ut5rnkxXcFVUSzyjLxQao'
        try:
            import random
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjozMzA3LCJwYXJ0IjoxODM5M30:1gGOCW:tfDBFZXbI2gByAiBBGRifWOp_7U'
        try:
            vh1 = ['4','7']
            vh = list(map(int, vh1))
            izh = [min(vh[0]*vh[1]/10, 10), vh[0] * vh[1]]
            with Check.input(vh1):
